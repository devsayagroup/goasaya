// // components/campaigns/WeekdaysClient.tsx
// "use client";
...
// export default function WeekdaySpecialsPage() {
...
//   return (
//     <main className="relative bg-black min-h-screen py-32 px-4 sm:px-6 md:px-12 font-serif selection:bg-stone-200 selection:text-black">
//         <div className="absolute inset-0">
//             <Image
//             src="/images/goa9.JPG" // Consider updating to .webp for performance!
//             alt="Goa Saya"
//             fill
//             priority
...
//             >
//             Curated Deals for Every Day of the Week
//             </motion.p>
//         </header>
...
//                               <a 
//                                 href={selectedPromo.waLink}
//                                 target="_blank"
//                                 rel="noopener noreferrer"
//                                 className="w-full block group relative overflow-hidden py-4 border border-black hover:border-black transition-colors duration-500 text-center"
//                               >
...
//             )}
//         </AnimatePresence>
//         <RelatedExperiences currentCampaignId="weekday-specials"/>
//     </main>
//   );
// }
...
import Image from 'next/image';
import { motion, AnimatePresence, Easing } from 'framer-motion';
import RelatedExperiences from '@/components/ui/RelatedExperiences'; // Adjusted path if needed
...
export default function WeekdaySpecialsClient() {
  const [selectedPromo, setSelectedPromo] = useState<typeof GOASAYA_WEEKDAYS[0] | null>(null);
  ...
  return (
    // Removed the padding and absolute positioning from the main wrapper
    <main className="w-full font-serif selection:bg-stone-200 selection:text-black flex flex-col">
      
      {/* SECTION 1: Weekday Specials Hero & Grid */}
      <section className="relative bg-black min-h-screen py-32 px-4 sm:px-6 md:px-12 overflow-hidden">
        {/* Background is now constrained to this section */}
        ...
        <div className="relative z-10 w-full">
          ...
          >
              ...
              <motion.div
                  ...
                  layoutId={`card-${promo.slug}`}
                  onClick={() => setSelectedPromo(promo)}
                  className="cursor-pointer group relative aspect-[4/5] overflow-hidden bg-stone-100 rounded-sm"
              >
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors duration-700 z-10 flex items-center justify-center">
                    <span className="text-white opacity-0 group-hover:opacity-100 transition-all duration-700 translate-y-4 group-hover:translate-y-0 uppercase tracking-widest text-xs font-sans font-bold mix-blend-overlay">
                        Explore
      ...
      {/* Modal is fixed to the viewport, so it sits cleanly outside the document flow */}
      <AnimatePresence>
          {selectedPromo && (
          <>
              <motion.div
      ...
      {/* SECTION 2: Related Experiences now sits perfectly beneath the dark section without padding overlaps */}
      <RelatedExperiences currentCampaignId="weekday-specials" />
    </main>
  );
}